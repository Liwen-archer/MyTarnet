...
def get_user_specified_hyperparameters(args):
    prop = {}
    prop['batch'], prop['lr'], prop['nlayers'], prop['emb_size'], prop['nhead'], prop['task_rate'], prop['masking_ratio'], prop['task_type'] = args.batch, args.lr, args.nlayers, args.emb_size, args.nhead, args.task_rate, args.masking_ratio, args.task_type
    return prop
...
def data_loader(dataset, task_type):
    X_train = np.load(os.path.join(dataset, 'X_train.npy'), allow_pickle = True).astype(np.float64)
    X_test = np.load(os.path.join(dataset, 'X_test.npy'), allow_pickle = True).astype(np.float64)
...
def preprocess(prop, X_train, y_train, X_test, y_test):
    ...
    batch, task_type = prop['batch'], prop['task_type']
    num_train_samples, num_test_samples = math.ceil(num_train_inst / batch) * batch, math.ceil(num_test_inst / batch) * batch
...
def initialize_training(prop):
    task_type, device, nclasses, seq_len, batch, input_size, emb_size, nhead, nhid, nhid_tar, nhid_task, nlayers, dropout, lr = prop['task_type'], prop['device'], prop['nclasses'], prop['seq_len'], prop['batch'], prop['input_size'], prop['emb_size'], prop['nhead'], prop['nhid'], prop['nhid_tar'], prop['nhid_task'], prop['nlayers'], prop['dropout'], prop['lr']
    model = MultitaskTransformerModel(task_type, device, nclasses, seq_len, batch, input_size, emb_size, nhead, nhid, nhid_tar, nhid_task, prop['nlayers'], dropout).to(device)
    best_model = MultitaskTransformerModel(task_type, device, nclasses, seq_len, batch, input_size, emb_size, nhead, nhid, nhid_tar, nhid_task, nlayers, dropout).to(device)
    
    criterion_tar = torch.nn.MSELoss()
    criterion_task = torch.nn.CrossEntropyLoss() if task_type == 'classification' else torch.nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr)
    best_optimizer = torch.optim.Adam(best_model.parameters(), lr)
...
def attention_sampled_masking_heuristic(X, masking_ratio, ratio_highest_attention, instance_weights):
    _, index = instance_weights.topk(int(math.ceil(ratio_highest_attention * X.shape[1])))
...
def compute_tar_loss(model, device, criterion_tar, y_train_tar_masked, y_train_tar_unmasked, batched_input_tar, batched_boolean_indices_masked, batched_boolean_indices_unmasked, num_inst, start):
    model.train()
    out_tar, _ = model(torch.as_tensor(batched_input_tar, device=device), 'reconstruction')
...
def  multitask_train(model, criterion_tar, criterion_task, optimizer, X_train_tar, X_train_task, y_train_tar_masked, y_train_tar_unmasked, y_train_task, boolean_indices_masked, boolean_indices_unmasked, prop):
    batch, nclasses, device = prop['batch'], prop['nclasses'], prop['device']
    task_type, task_rate, lamb = prop['task_type'], prop['task_rate'], prop['lamb']
    model.train()
    total_loss_tar_masked, total_loss_tar_unmasked, total_loss_task = 0.0, 0.0, 0.0
    num_batches = math.ceil(X_train_task.shape[0] / batch)
    output, attn_arr = [], []
    ...
    for i in range(num_batches):
        ...
        batched_input_tar = X_train_task[start:end]
        ...
        loss_tar_masked, loss_tar_unmasked = compute_tar_loss(model, device, criterion_tar, y_train_tar_masked, y_train_tar_unmasked, batched_input_tar, batched_boolean_indices_masked, batched_boolean_indices_unmasked, num_inst, start)
        
        attn, loss_task = compute_task_loss(nclasses, model, device, criterion_task, y_train_task, batched_input_task, task_type, num_inst, start)
        
        total_loss_tar_masked += loss_tar_masked.item()
        total_loss_tar_unmasked += loss_tar_unmasked.item()
        total_loss_task += loss_task.item()
        
        loss = task_rate * (lamb * loss_tar_masked + (1 - lamb) * loss_tar_unmasked) + (1 - task_rate) * loss_task
        loss.backward()
        optimizer.step()
...
def test(model, X, y, batch, nclasses, criterion, task_type, device, avg):
    ...
    return evaluate(torch.cat(output_arr, axis=0), y, nclasses, criterion, task_type, device, avg)
...
def training(model, optimizer, criterion_tar, criterion_task, best_model, best_optimizer, X_train_task, y_train_task, X_test, y_test, prop):
    ...
    seq_len, device, epoches, masking_ratio, ratio_highest_attention, avg, task_type, batch, nclasses = prop['seq_len'], prop['device'], prop['epochs'], prop['masking_ratio'], prop['ratio_highest_attention'], prop['avg'], prop['task_type'], prop['batch'], prop['nclasses']
      
    instance_weights = torch.as_tensor(torch.rand(X_train_task.shape[0], seq_len), device=device)

    for epoch in range(epoches):
        X_train_tar, y_train_tar_masked, y_train_tar_unmasked, boolean_indices_masked, boolean_indices_unmasked = random_instance_masking(X_train_task, masking_ratio, ratio_highest_attention, instance_weights)
        ...
        print('Epoch: ' + str(epoch + 1) + ', TAR Loss: ' + str(tar_loss), ', TASK Loss: ' + str(task_loss))
    ...
    metrics = test(best_model, X_train_task, y_train_task, batch, nclasses, criterion_task, task_type, device, avg)
    if task_type == 'classification':
        print('TEST ===> Dataset: ' + prop['dataset'] + ', Acc: ' + str(metrics[1]) + ', Prec: ' + str(metrics[2]) + ', Rec: ' + str(metrics[3]) + ', F1: ' + str(metrics[4]))
    else:
